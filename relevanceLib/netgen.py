...
def function_tests():
    ...
    if os.path.isfile(gname):
        NG.load_graph(gname=gname)
    else:
        NG.load_graph(fname=fname)
        NG.write_graph(gname)
        
    print('The number of venue nodes: ', len(NG.get_nodes_by_type('venue')))
    print('The number of placeness nodes: ', len(NG.get_nodes_by_type('pl')))
    print('The number of user nodes: ', len(NG.get_nodes_by_type('user')))